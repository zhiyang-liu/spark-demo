...
object ChapterSixExample {
  def main(args: Array[String]) {
    ...
    callSigns.count()//惰性求值，必须加行动操作才会执行
    println("dataLine:" + dataLines.value + "errorLines:" + errorLines.value)
    ...
    if (invalidSignCount.value < 0.5 * validSignCount.value) {
      ...
      println("invalidSignCount:" + invalidSignCount.value + "validSignCount:" + validSignCount.value)
    } else {
    ...
    val countryCounts2 = sc.textFile(inputFile2)
      ...
      .map{case sign =>
      val country = lookupInArray(sign, signPrefixes.value)
      (country, 1)}.reduceByKey((x, y) => x + y).collect().foreach(println)
    ...
    println(contactsContactLists.values)
    ...
    /*val distScript = "./files/finddistance.R"
